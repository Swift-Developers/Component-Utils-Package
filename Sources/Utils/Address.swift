...
public enum Address {
    ...
    public static let list: [Model] = {
        ...
        do {
            return try JSONDecoder().decode([Address.Model].self, from: data)
        } catch {
            return []
        }
    } ()
...
extension Address {
    ...
    /// 地区级别
    public enum Grade {
        case province   // 省(直辖市)   1级
        case city       // 市级        2级
        case county     // 县(区)级    3级
    ...
    /// - Parameter grade: 等级
    /// - Parameter code: 邮编码
    public static func get(_ grade: Grade, code: Int) -> [String]? {
        return .init(get(names: code)?.prefix(grade.level) ?? [])
    }
}
...
public extension Address {
    ...
    static func get(code name: String) -> Int? {
        return Int(get(models: name)?.last?.code ?? "")
    }
...
extension Address {
    ...
    public struct Model: Codable {
        public let children: [Model]?
        public let code: String
        public let name: String
    }
}

/// json数据 (由于SPM不支持资源文件 暂时放到这里)
private let json = """
[{
    "children":[{
        "children": [{
