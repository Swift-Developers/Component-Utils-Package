...
public extension UserDefaultsSettable where Keys.RawValue == String {
    ...
    static func observe(forKey key: Keys, change: @escaping (Any?, Any?) -> Void) -> DefaultsObservation {
        return DefaultsObservation(key: forKey(key), onChange: change)
    }
    
    static func observe<Old, New>(forKey key: Keys, change: @escaping (Old?, New?) -> Void) -> DefaultsObservation {
        return DefaultsObservation(key: forKey(key)) { old, new in
            change(old as? Old, new as? New)
        }
    }
    
    static func observe<T: Decodable>(forKey key: Keys, change: @escaping (T?, T?) -> Void) -> DefaultsObservation {
        return DefaultsObservation(key: forKey(key)) { old, new in
